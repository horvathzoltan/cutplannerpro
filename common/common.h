...
inline QString categoryToString(ProfileCategory cat) {
    if (cat == ProfileCategory::RollerTube) return "RollerTube";
    if (cat == ProfileCategory::BottomBar)  return "BottomBar";
    return "Unknown";
}

inline ProfileCategory categoryFromString(const QString& str) {
...
inline QString badgeColorForCategory(ProfileCategory cat) {
