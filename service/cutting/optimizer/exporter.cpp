...
//         QStringList segmentLabels = plan._segments.toLabelList();
...
//             << plan._segments.wasteInfo().length << ","
...
//         out << QString("Kerf összesen: %1 mm\n").arg(plan._segments.kerfInfo().length);
//         out << QString("Hulladék: %1 mm\n").arg(plan._segments.wasteInfo().length);
